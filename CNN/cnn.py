...
for epoch in range(num_epochs):
    for i, batch in enumerate(train_loader):
        article, question, options, labels = batch

        #x = article.to(device, dtype = torch.float32)
        y = labels.to(device)
    ...
    output_model_file = os.path.join('./', "pytorch_model_{}epochs.bin".format(epoch))
    torch.save(model.state_dict(), output_model_file)
    accuracy = 0

for i, batch in enumerate(dev_loader):
    article, question, options, labels = batch
    print(i/len(dev_loader))
...
print('Accuracy of test set {}'.format(accuracy/len(test_loader.dataset)))
